...
public final class SetImplSceneTerrainGridContainer implements
		SceneTerrainGridContainer {
	private static final Random random = new Random(System.currentTimeMillis());

	private List<SceneGrid> allGrids;
	private List<SceneGrid> unblockedGrids;

	public SetImplSceneTerrainGridContainer(Collection<SceneGrid> grids) {
		setContainsSet(grids);
	}
	...
	private void setContainsSet(Collection<SceneGrid> grids) {
		this.allGrids = new ArrayList<>(grids);

		ArrayList<SceneGrid> temp = new ArrayList<>(allGrids.size());
		for (SceneGrid grid : allGrids) {
			if (!grid.isBlocked()) {
				temp.add(grid);
			}
		}
		this.unblockedGrids = new ArrayList<>(temp);
	}
