package newbee.morningGlory.checker.refObjectChecker.quest;
...
public class QuestRefChecker extends BaseRefChecker<QuestRef> {
	...
	public void check(GameRefObject gameRefObject) {
		QuestRef info = (QuestRef) gameRefObject;
		if (!info.getId().startsWith("quest_")) {
			error(gameRefObject, "主线任务<refId>错误 , 请以quest_开头!!! 错误的refId为: " + info.getId());
		}

		checkQuestPropertyData(gameRefObject);
		checkQuestCondition(gameRefObject);
		checkQuestRefOrder(gameRefObject);
		checkQuestRefReward(gameRefObject);
		checkQuestRefNpc(gameRefObject);
	}

	private void checkQuestPropertyData(GameRefObject gameRefObject) {
		QuestRef ref = (QuestRef) gameRefObject;
		PropertyDictionary pd = ref.getProperty();
		String preQuestId = MGPropertyAccesser.getPreQuestId(pd);
		if (!StringUtils.isEmpty(preQuestId)) {
			if (!preQuestId.startsWith("quest_")) {
				error(gameRefObject, "主线任务<preQuestId>错误 , 请以 quest_ 开头!!! 错误的preQuestId为: " + preQuestId);
			}
		}

		String nextQuestId = MGPropertyAccesser.getNextQuestId(pd);
		if (!StringUtils.isEmpty(nextQuestId)) {
			if (!nextQuestId.startsWith("quest_")) {
				error(gameRefObject, "主线任务<nextQuestId>错误 , 请以 quest_ 开头!!! 错误的nextQuestId为: " + nextQuestId);
			}
		}

		int questType = MGPropertyAccesser.getQuestType(pd);
		if (questType != 1) {
			error(gameRefObject, "主线任务<questType>错误 , 请将 questType 修改为 1 !!! 错误的questType为: " + questType);
		}
	}

	private void checkQuestRefOrder(GameRefObject gameRefObject) {
		QuestRef ref = (QuestRef) gameRefObject;
		QuestRefOrder questOrder = ref.getOrder();
		...
		for (QuestRefOrderItem order : orders) {
			...
			if (orderType == QuestRefOrderType.Kill_Monster_Order_Type) {
				...
				if (!killMonster.startsWith("monster_")) {
					error(gameRefObject, "主线任务<orderField>错误 , monsterRefId错误，请修改为monster_开头 !!! 错误的monsterRefId为: " + killMonster);
				}
				if (GameRoot.getGameRefObjectManager().getManagedObject(killMonster) == null) {
					error(gameRefObject, "主线任务<orderField>错误, 怪物RefId不存在！！！错误的怪物RefId为" + killMonster);
				}
				int killCount = kmOrder.getNumber();
				if (killCount <= 0) {
					error(gameRefObject, "主线任务<orderField>错误 , killCount错误，killCount不能小于等于0 !!! 错误的killCount为: " + killCount);
				}
			}
			if (orderType == QuestRefOrderType.Collect_Order_Type) {
				...
				if (GameRoot.getGameRefObjectManager().getManagedObject(itemId) == null) {
					error(gameRefObject, "主线任务<orderField>错误, 采集物产生道具RefID不存在！！！错误的itemRefId为" + itemId);
				}
				String npcRefId = collectOrder.getCollectTargetObjectId();
				if (!npcRefId.startsWith("npc_collect_")) {
					error(gameRefObject, "主线任务<orderField>错误, 采集物产生 NPC 请以 npc_collect_ 开头！！！错误的npcRefId为" + npcRefId);
				}
				int itemCount = collectOrder.getNumber();
				if (itemCount <= 0) {
					error(gameRefObject, "主线任务<orderField>错误 , itemCount错误，itemCount不能小于等于0 !!! 错误的itemCount为: " + itemCount);
				}
			}
			if (orderType == QuestRefOrderType.Talk_Order_Type) {
				...
				if (GameRoot.getGameRefObjectManager().getManagedObject(npcId) == null) {
					error(gameRefObject, "主线任务<orderField>错误, NpcRefID不存在！！！错误的NpcRefID为" + npcId);
				}
			}
		}
	...
	private void checkQuestRefReward(GameRefObject gameRefObject) {
		QuestRef ref = (QuestRef) gameRefObject;
		QuestRefReward rewardRef = ref.getReward();
	...
	private void questRefWardItemChecker(GameRefObject gameRefObject, QuestRefRewardItem rewardItem) {
		...
		if (GameRoot.getGameRefObjectManager().getManagedObject(itemRefId) == null) {
			error(gameRefObject, "主线任务<rewardField>错误 , itemRefId不存在 !!! 错误的itemCount为: " + itemRefId);
		}
	}
		...
		if (propertyRewardList.size() > 0) {
			for (int i = 0; i < propertyRewardList.size(); i++) {
				if (propertyRewardList.get(i).getId() == MGPropertySymbolDefines.Exp_Id) {
					...
					if (itemNumber < 0) {
						error(gameRefObject, "主线任务<rewardField>错误 , Exp 数量 !!! 错误的数量为: " + itemNumber);
					}
				}
				// 金币
				if (propertyRewardList.get(i).getId() == MGPropertySymbolDefines.Gold_Id) {
					int itemNumber = (Integer) propertyRewardList.get(i).getValue();
					if (itemNumber < 0) {
						error(gameRefObject, "主线任务<rewardField>错误 , Gold 数量 !!! 错误的数量为: " + itemNumber);
					}
				}
				// 绑定元宝
				if (propertyRewardList.get(i).getId() == MGPropertySymbolDefines.BindedGold_Id) {
					int itemNumber = (Integer) propertyRewardList.get(i).getValue();
					if (itemNumber < 0) {
						error(gameRefObject, "主线任务<rewardField>错误 , BindedGold 数量 !!! 错误的数量为: " + itemNumber);
					}
				}
				// 非绑元宝
				if (propertyRewardList.get(i).getId() == MGPropertySymbolDefines.UnbindedGold_Id) {
					int itemNumber = (Integer) propertyRewardList.get(i).getValue();
					if (itemNumber < 0) {
						error(gameRefObject, "主线任务<rewardField>错误 , UnbindedGold 数量 !!! 错误的数量为: " + itemNumber);
					}
				}
				// 功勋
				if (propertyRewardList.get(i).getId() == MGPropertySymbolDefines.Merit_Id) {
					int itemNumber = (Integer) propertyRewardList.get(i).getValue();
					if (itemNumber < 0) {
						error(gameRefObject, "主线任务<rewardField>错误 , Merit 数量 !!! 错误的数量为: " + itemNumber);
					}
				}
				// 成就
				if (propertyRewardList.get(i).getId() == MGPropertySymbolDefines.Achievement_Id) {
					int itemNumber = (Integer) propertyRewardList.get(i).getValue();
					if (itemNumber < 0) {
						error(gameRefObject, "主线任务<rewardField>错误 , Achievement 数量 !!! 错误的数量为: " + itemNumber);
					}
				}
			}
		}
	...
	private void checkQuestRefNpc(GameRefObject gameRefObject) {
		...
		if (!StringUtils.isEmpty(acceeptNpc)) {
			if (GameRoot.getGameRefObjectManager().getManagedObject(acceeptNpc) == null) {
				error(gameRefObject, "主线任务<npcField>错误 , acceeptNpc不存在 !!! 错误的acceeptNpc为: " + acceeptNpc);
			}
		}
		if (!StringUtils.isEmpty(submitNpc)) {
			if (GameRoot.getGameRefObjectManager().getManagedObject(submitNpc) == null) {
				error(gameRefObject, "主线任务<npcField>错误 , submitNpc不存在 !!! 错误的submitNpc为: " + submitNpc);
			}
		}
	}
	...
	private void checkQuestCondition(GameRefObject gameRefObject) {
		QuestRef ref = (QuestRef) gameRefObject;
		Set<QuestRefCondition> conditionSet = ref.getConditionSet();
		for (QuestRefCondition condition : conditionSet) {
			SingleQuestCondition singleCondition = (SingleQuestCondition) condition;
			...
			for (QuestRefConditionItem item : itemSet) {
				...
				if (item.getType() != visibleType && item.getType() != acceptType) {
					error(gameRefObject, "主线任务<conditionField>错误 , conditionType 只有 0 和 1 两种类型!!! 错误的conditionType为: " + item.getType());
				}
			}
		}
	}
